...
function validateAccessoryGroupNoValue(value, internalNo){
    value = (value || '').trim();
    if(!value){
        return '';
    }
    internalNo = (internalNo || '').trim();
    if(internalNo && internalNo.includes('-')){
        return '';
    }
...
function confirmDeviceSave(form){
    ...
    const groupNo = ((form.querySelector('[name="group_no"]') || {}).value || '').trim();
    const internalNo = ((form.querySelector('[name="internal_no"]') || {}).value || '').trim();
    if(groupNo){
        const msg = deviceType === '配件' ? validateAccessoryGroupNoValue(groupNo, internalNo) : validateAssetGroupNoValue(groupNo);
