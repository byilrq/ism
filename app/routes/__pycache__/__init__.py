...
try:
    from .cable import register_cable_routes
    ...
    if not keyword:
        ...
        for item in asset_rows:
            rows.append({
                ...
                "location_detail_url": url_for("asset_location_detail", location=item.location) if normalize_text(item.location) else "",
                "parent_asset_id": "",
    ...
    if asset_ids:
        ...
        for asset in matched_assets:
            ...
            rows.append({
                ...
                "location_detail_url": url_for("asset_location_detail", location=asset.location) if normalize_text(asset.location) else "",
                "parent_asset_id": "",
...
def register_routes(app):
    ...
    @app.route("/asset/location", methods=["GET"])
    @login_required
    def asset_location_detail():
        ...
        assets = Asset.query.filter(Asset.location == location).order_by(Asset.internal_no.asc(), Asset.group_no.asc(), Asset.id.asc()).all()
        return render_template_string(
            ASSET_LOCATION_HTML,
            current_user=current_user,
            location=location,
...
<div class="wrap">
    ...
    <div class="card">
        <div class="switch-row">
            <a href="/"><button type="button">资产查询</button></a>
        ...
        <form method="post" action="/import_devices" enctype="multipart/form-data" style="margin-top:12px;">
            ...
            <div class="muted" style="margin-top:8px;">Excel格式：集团编号、内部编号、名称、型号、责任人、位置、状态、备注、类型（主设备/配件）</div>
        </form>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ item.name }}</td>
                            <td>{% if item.location_detail_url %}<a class="num-link" href="{{ item.location_detail_url }}">{{ item.location }}</a>{% else %}{{ item.location }}{% endif %}</td>
                            <td>{{ item.model }}</td>
                            <td>{{ item.status }}</td>
                            <td>{{ item.owner }}</td>
...
.switch-row a{flex:1 1 160px;}
.switch-row a button{width:100%;}
button{padding:10px;font-size:16px;border-radius:6px;border:1px solid #ccc;box-sizing:border-box;background:#0d6efd;color:#fff;cursor:pointer;}
.btn-gray{background:#6c757d;}
.table-wrap{overflow-x:auto;}
table{width:100%;border-collapse:collapse;min-width:900px;}
th,td{border:1px solid #ccc;padding:8px;text-align:left;}
...
<div class="wrap">
    ...
    <div class="card">
        ...
        <div style="margin-bottom:8px;"><a href="/">返回资产查询页</a></div>
        <h2 style="margin:0;">位置：{{ location }}</h2>
    </div>
        ...
        <div class="table-wrap">
            <table>
                ...
                <tbody>
                    {% for item in assets %}
                    <tr>
                        <td><a class="num-link" href="/asset/{{ item.id }}">{{ item.group_no or '' }}</a></td>
                        <td><a class="num-link" href="/asset/{{ item.id }}">{{ item.internal_no or '' }}</a></td>
                        <td>{{ item.name }}</td>
                        <td>{{ item.model or '' }}</td>
                    ...
                    {% endfor %}
                    {% if not assets %}
                    <tr><td colspan="7">该位置下暂无主设备</td></tr>
                    {% endif %}
                </tbody>
            </table>
        </div>
...
.confirm-dialog{position:fixed;inset:0;background:rgba(0,0,0,.45);display:none;align-items:center;justify-content:center;z-index:10000;padding:16px;}
.confirm-dialog.show{display:flex;}
.confirm-dialog-card{width:100%;max-width:360px;background:#fff;border-radius:14px;padding:18px;box-shadow:0 8px 24px rgba(0,0,0,.18);}
.confirm-dialog-title{font-size:18px;font-weight:bold;margin-bottom:10px;text-align:center;}
.confirm-dialog-message{color:#333;line-height:1.6;margin-bottom:16px;text-align:center;word-break:break-word;}
.confirm-dialog-actions{display:flex;justify-content:center;gap:12px;flex-wrap:wrap;}
.confirm-dialog-actions button{width:auto;min-width:120px;}
@media (max-width:768px){.grid{grid-template-columns:1fr;}}
</style>
<script>
...
function openDeleteConfirm(formId, message){
    ...
    if(messageEl){
        messageEl.textContent = message || '确认删除吗？';
    }
    if(dialog){
        dialog.classList.add('show');
    }
...
function confirmDeleteSubmit(){
    const formId = pendingDeleteFormId;
    closeDeleteConfirm();
    if(!formId){
        return;
    }
    ...
    <div class="card">
        ...
        <form id="asset-form" method="post" enctype="multipart/form-data" onsubmit="return confirmAssetSave(this)">
            ...
            <div class="detail-actions">
                ...
                <button type="submit" id="asset-form-save" style="display:none;">确认</button>
            </div>
        </form>

        <form id="asset-delete-form" method="post" action="/asset/{{ asset.id }}/delete" onsubmit="return openDeleteConfirm('asset-delete-form', '确认删除该主设备及其所有配件？');" style="display:none;"></form>
    </div>

    <div id="delete-confirm-dialog" class="confirm-dialog" onclick="if(event.target === this){closeDeleteConfirm();}">
        <div class="confirm-dialog-card">
            <div class="confirm-dialog-title">删除确认</div>
            <div id="delete-confirm-message" class="confirm-dialog-message">确认删除吗？</div>
            <div class="confirm-dialog-actions">
                <button type="button" class="btn-gray" onclick="closeDeleteConfirm()">取消</button>
                <button type="button" class="btn-red" onclick="confirmDeleteSubmit()">确认删除</button>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <form id="accessory-form" method="post" enctype="multipart/form-data" onsubmit="return confirmAccessorySave(this)">
            ...
            <div class="detail-actions">
                ...
                <button type="submit" id="accessory-form-save" style="display:none;">确认</button>
            </div>
        </form>

        <form id="accessory-delete-form" method="post" action="/accessory/{{ accessory.id }}/delete" onsubmit="return openDeleteConfirm('accessory-delete-form', '确认删除该配件？');" style="display:none;"></form>
    </div>
    <div id="delete-confirm-dialog" class="confirm-dialog" onclick="if(event.target === this){closeDeleteConfirm();}">
        <div class="confirm-dialog-card">
            <div class="confirm-dialog-title">删除确认</div>
